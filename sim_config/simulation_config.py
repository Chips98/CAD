...
CONDITIONAL_EVENTS = {
    "low_grades": {
        "condition": lambda state: state.get("recent_grades", []) and sum(state["recent_grades"]) / len(state["recent_grades"]) < 70,
        "events": [
            "{father}看到成绩单后非常失望",
    ...
    "high_stress": {
        "condition": lambda state: state.get("stress_level", 0) > 7,
        "events": [
